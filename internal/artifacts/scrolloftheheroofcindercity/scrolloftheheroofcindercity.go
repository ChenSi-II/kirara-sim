...
var reactToElements = map[info.ReactionType][]attributes.Element{
	...
	info.ReactionTypeStarSuperconduct:   {attributes.Electro, attributes.Cryo},
	...
	info.ReactionTypeStarDiffusion:      {attributes.Anemo, attributes.Cryo},
...
func NewSet(c *core.Core, char *character.CharWrapper, count int, param map[string]int) (info.Set, error) {
	...
	if count >= 4 {
		for evt, react := range map[event.Event]info.ReactionType{
			...
			event.OnStarSuperconduct:   info.ReactionTypeStarSuperconduct,
			...
			event.OnStarDiffusion:      info.ReactionTypeStarDiffusion,
