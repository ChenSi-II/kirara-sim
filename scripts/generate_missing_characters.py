...
def write_catalog(entries: list[dict]) -> None:
    ...
    for entry in entries:
        ...
        modifiers = stats["stats_modifier"]
        curves = {
            item["type"]: item["grow_curve"]
            for item in modifiers["prop_grow_curves"]
        }
        promo_levels = (40, 50, 60, 70, 80, 90)
        promo_blocks = ["\t\t\t\t{MaxLevel: 20},"]
        for level, values in zip(promo_levels, modifiers["ascension"], strict=True):
            props = []
            for prop, value in values.items():
                ...
                props.append(
                    "\t\t\t\t\t"
                    f"{{PropType: model.FightPropType_{enum}, Value: {value}}},"
                )
            promo_blocks.append(
                f"\t\t\t\t{{MaxLevel: {level}, AddProps: []*model.PromotionAddProp{{\n"
                + "\n".join(props)
                + "\n\t\t\t\t}},"
            )
...
\t\t\tBaseHp: {stats['base_hp']}, BaseAtk: {stats['base_atk']}, BaseDef: {stats['base_def']}, ElementMastery: {stats.get('elemental_mastery', 0)},
\t\t\tHpCurve: model.GrowCurveType_{curves['FIGHT_PROP_BASE_HP']},
\t\t\tAtkCurve: model.GrowCurveType_{curves['FIGHT_PROP_BASE_ATTACK']},
\t\t\tDefCruve: model.GrowCurveType_{curves['FIGHT_PROP_BASE_DEFENSE']},
\t\t\tPromoData: []*model.PromotionData{{
{chr(10).join(promo_blocks)}
\t\t\t}},
\t\t}},
...
def main() -> None:
    ...
    parser.add_argument(
        "--catalog-only",
        action="store_true",
        help="regenerate only the community character catalog without touching packages or manifest state",
    )
    ...
    if args.catalog_only:
        ...
        print(f"catalog-generated={len(entries)}")
        return
    for entry in entries:
        write_package(entry)
